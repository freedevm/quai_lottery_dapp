...
export default function PreLoading({
  ...
  // Automatically open Web3Modal when wallet is not connected
  useEffect(() => {
    if (!isConnected) {
      open();
    }
      ...
      <div className="w-full h-screen relative flex justify-center items-center bg-purple-950 text-2xl overflow-hidden">
        ...
        <div className="w-full h-full text-center absolute top-[85%]">
          <div>
            ...
            <span className="font-extrabold text-[#DADADA] text-[14px] leading-[10.47px] font-[Monument] fade-in ml-2">
              Ethereum Lottery
            </span>
          </div>
        </div>
        {isConnected && dataFetched ? (
              ...
              {isConnected ? (
                <div className="text-center">
                  <div role="status">
                    <svg
                      aria-hidden="true"
                      className="inline w-8 h-8 text-purple-700 animate-spin dark:text-gray-600 fill-white"
                      viewBox="0 0 100 101"
                      fill="none"
